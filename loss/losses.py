...
class MyLoss(nn.Module):
    ...
    def forward(self, xs, ys):
        ...
        charloss_temp = 0.8 * self.loss(xs, ys)
        color_loss = 0.2 * self.colorloss(xs, ys)
        # L_total = L1_temp + L2_temp
        # L_total = L1_temp+color_loss #1.25
        L_total = L1_temp+color_loss +charloss_temp#all
        # L_total = charloss_temp +color_loss
        return L_total
...
class ranker_loss(nn.Module):
    def __init__(self):
        super(ranker_loss,self).__init__()
        self.iqa_uranker = pyiqa.create_metric('uranker', as_loss=True).cuda()
    def forward(self,img):
        score = self.iqa_uranker(img)
        loss = torch.mean(F.sigmoid(-score))
        return loss
